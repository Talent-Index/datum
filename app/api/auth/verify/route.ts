...
/** Check the code and, if it holds, start a session bound to the number. */
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!row || row.expiresAt.getTime() < Date.now()) {
    return NextResponse.json({ error: "No live code for this number; request a new one" }, { status: 400 });
  }
  ...
  if (row.codeHash !== hashOtp(phone, parsed.data.code)) {
    await database
  ...
  setSenderCookie(response, issueSenderToken(phone));
  return response;
}
